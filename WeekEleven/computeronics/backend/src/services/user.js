...
const isAuthorizeUpdateUser = async (headers, userType = CUSTOMER) => {
  ...
  const tokenData = await tokenService.getAuthData(
    headers["authorization"].split(" ")[1]
  );
  const tokenDataUserType =
    tokenData && tokenData.data && tokenData.data.userType;
  isAuthorized =
    tokenData && USER_TYPE[tokenDataUserType].USER_RIGHTS.includes(userType);
  return isAuthorized;
};
const isPasswordUser = (userType = CUSTOMER) =>
  userType === ADMIN || userType === SUPER_ADMIN || userType === TICKET_MANAGER;
...
const saveUser = async ({ body: data, headers }) => {
  ...
  try {
    const isAuthoriseToCreateUser = await isAuthorizeUpdateUser(
      headers,
      data.userType
    );
    const user =
      data.contact && (await User.findOne({ contact: data.contact }));
    if (user && !user.status) {
      message = "There is some problem with this user. Please contact admin";
    } else if (user) {
      message =
        "There is already an active user found with this contact number";
...
const getUser = async ({ headers }) => {
  ...
  console.log(hash, "=======");
  try {
    let user = await tokenService.getAuthData(
      headers["authorization"].split(" ")[1]
    );
...
const deleteUser = async (req) => {
  ...
  try {
    ...
    const isAuthoriseToDeleteUser = await isAuthorizeUpdateUser(
      headers,
      user.userType
    );
    if (!user || !user.status) {
...
const updateUser = async (req) => {
  ...
  try {
    ...
    const headers = req.headers;
    let user = await User.findById(userId);
    const isAuthoriseToUpdateUser = await isAuthorizeUpdateUser(
      headers,
      user.userType
    );
    if (!user) {
    ...
    if (!error) {
      responseData = await User.findByIdAndUpdate(req.params.id, req.body, {
        new: true,
      });
...
const authenticateUser = async (data) => {
  ...
  try {
    ...
    if (user && (isPasswordUser(user.userType) || data.isEmployee)) {
      let isMatch =
        data.password && (await bcrypt.compare(data.password, user.password));
      if (isMatch) {
...
const getUserById = async (req) => {
  ...
  try {
    ...
    const headers = req.headers;
    let user = await User.findById(userId);
    const isAuthoriseToViewUser = await isAuthorizeUpdateUser(
      headers,
      user.userType
    );
    if (!user || !user.status) {
